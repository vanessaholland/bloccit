...
RSpec.describe SessionsController, type: :controller do
  let(:my_user) { User.create!(name: "Blocer", email: "vanessa@bloc.io", password: "password") }
  ...
  describe "POST sessions" do
    it "returns http success" do
      post :create, usersession: {email: my_user.email}
      expect(response).to have_http_status(:success)
    end

    it "initializes a session" do
      post :create, usersession: {email: my_user.email, password: my_user.password}
    ...
    it "flashes #error with bad email address" do
      post :create, usersession: {email: "does not exist"}
